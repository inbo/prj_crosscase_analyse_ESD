...
model_indivmeans <- readRDS("models/respondent_weighted_means.RDS")


```

## Analyses op basis van het gemiddelde

### similariteitsprofiel tusssen de cases en ecosysteemdiensten

Het similariteitsprofiel is hier als volgt gedefinieerd:

-   Neem het gemiddelde per case per ess -Normeer alle waarden door het gemiddelde over de cases respectievelijk ess heen af te trekken
...
Uit figuur @fig-similaritycase blijkt dat hoe ecosysteemdiensten gescoord worden voor redelijk veel cases vrij hoog is, en dan vooral voor alles in de Wijers.

De case Dijle en Voeren blijken wel sterk van elkaar af te wijken in hoe ecosysteemdiensten worden geëvalueerd, en blijken zelf ook heel verschillend van de andere ecosysteemdiensten.
...
Uit figuur @fig-similarityess blijken jacht en gemotoriseerde recreatie sterk te verschillen qua beoordeling dan de meeste andere ecosysteemdiensten, niet onlogisch omdat dit nu ook de 2 zijn die over het algemeen als negatief ervaren worden.

Een analoge conclusie kqn getrokken worden uit figuur @fig-regionalnetworks, waar duidelijk is dat er heel weinig samenhang is tussen jacht en de rest, tussen gemotoriseerde recreatie en de rest en ook tussen dynamische recreatie en de rest.

```{r fig-regionalnetworks}
#| fig-cap: "Connectedness of the ecosystem services"
library(igraph)
library(ggraph)
...
NMDS schaling in figuur @fig-nmds toont vergelijkhare informatie, waar hier ook de jacht en gemotoriseerde recreatie sterk verschillen vqn de rest. De dynamische recreatie is hier iets minder uitgesproken. Aan de andere kant van het spectrum heb je dan de levenskwaliteit, de iconische soorten en in mindere mate verbetering van vervuiling.
...
#| fig-cap: "Leik agreement between the ecosystem services per case"
#| fig-height: 10
#| fig-width: 7
ggplot(data_consensus, aes(x = ess_short, y = consensus, color = cluster_short)) +
  geom_point() +
  facet_wrap(~case_short, ncol = 3) +
...
Uit figuur @fig-consensus-detail blijkt dat de consensus over de beoordeling van de ecosysteemdiensten vaak vrij hoog ligt, maar dat er wel verschillen zijn tussen de cases, vooral in Voeren.


Indien de consensus wordt uitgemiddeld over de cases heen zoals in figuur @fig-consensus-marginal, dan blijkt de consensus zich redelijk uit te middellen rond ongeveer 0.60 tussen de ecosysteemdiensten met de meeste overeenkomst voor biodiversiteit, en de minste overeenkomst rond drinkwater.

```{r fig-consensus-marginal}
#| fig-cap: "Leik agreement between the ecosystem services over the cases"
...
Uit figuur @fig-swm_res blijken er geen sterke residuele effecten aanwezig te zijn.
...
Figuur @fig-swm_predwithconsensus toont dezelfde informatie, maar nu ook met toevoeging van een maat van consensus. Gemotoriseerde recreatie wordt als negatief aanzien en er is een grote consensus over, jacht minder negatief maar ook met minder consensus. Waterkwaliteit, iconische soorten en biodiversiteit scoren het hoogst en hebben een hoge consensus.

Er dient wel opgemerkt te worden, dat consensus ook we een stuk afhangt van de gemiddelde score. Om een extreme score te hebben, positief of negatief, is er veel consensus nodig, anders wordt het gemiddelde naar het midden getrokken.


Alternatieve voorstellingswijze:

```{r fig-consensus-desir}
#| fig-cap: "Consensus plotted against desirability"

p <- ggplot(data = pred_with_css, 
            aes(x = mean,
                y = consensus, 
                text = ess_short)) +
  geom_point(aes(color = cluster_short)) 
...
```{r fig-desir-ord1}
#| fig-cap: "Estimated effect and agreement per ESS ordered by mean effect"

ggplot(pred_with_css, aes(x = reorder(ess_short, mean), y = mean, color = consensus)) +
  geom_point(aes(size = consensus), alpha = 1) +
  geom_errorbar(aes(ymin = lcl, ymax = ucl)) +
  coord_flip() +
  labs(title = NULL,
...
```{r fig-desir-ord2}
#| fig-cap: "Estimated effect and agreement per ESS ordered by consensus"

library(ggplot2)

ggplot(pred_with_css, aes(x = reorder(ess_short, consensus), y = consensus, color = mean)) +
  ...
  labs(title = NULL,
       ...
       size = "Estimated score") +
  scale_color_gradient(low = "cyan", high = "blue") +
  scale_size_continuous(range = c(2, 4))

```

```{r fig-multiple-axes-ordermean}
#| fig-cap: "Estimated effect and agreement per ESS common"

library(ggplot2)
library(dplyr)

# 1. Define the desired ranges
prim_min <- -0.4; prim_max <- 2.6
sec_min  <- 0.3;  sec_max  <- 0.8

# 2. Calculate the scaling math (Linear Transformation: y = mx + c)
# This maps the secondary range onto the primary range
slope <- (prim_max - prim_min) / (sec_max - sec_min)
intercept <- prim_min - slope * sec_min

plotdata <- pred_with_css |> 
  mutate(ess_short = reorder(ess_short, mean))

ggplot(plotdata, aes(x = ess_short, group = 1)) +
  # Primary Line (Mean)
  geom_line(aes(y = mean, color = "Estimated Score"), linewidth = 1) +
  
  # Secondary Line (Consensus) - Transformed to fit primary scale
  geom_line(aes(y = consensus * slope + intercept, color = "Agreement"), linewidth = 1) +
  ...
  # Formatting
  labs(x = "ESS Common", color = "Metric") +
  theme_minimal() +
  theme(
...
```{r fig-multiple-axes-orderagreement}
#| fig-cap: "Estimated effect and agreement per ESS common (Ordered by Agreement)"
...
# Update reorder to use 'consensus' instead of 'mean'
plotdata <- pred_with_css |> 
  mutate(ess_short = reorder(ess_short, consensus))

ggplot(plotdata, aes(x = ess_short, group = 1)) +
  # Primary Line (Mean)
  geom_line(aes(y = mean, color = "Estimated Score"), linewidth = 1) +
  
  # Secondary Line (Consensus)
  geom_line(aes(y = consensus * slope + intercept, color = "Agreement"), linewidth = 1) +
...
```{r tbl-indivmean-summary}

emm_ess <- emmeans(model_indivmeans, specs = ~ ess_short)
tabel_fixefi <- as.data.frame(emm_ess) |> 
  transmute(par = ess_short, mean = emmean, lcl = lower.HPD, ucl = upper.HPD)

sigma <- summary(model_indivmeans)$spec_pars |> slice(1)
sdint <- summary(model_indivmeans)$random$case_short |> slice(1)
sdgrp <- summary(model_indivmeans)$random$`case_short:respondent` |> slice(1)
tabel_ranefi <- bind_rows(
  sd_intercept = sdint,
  sd_nested = sdgrp,
  sigma = sigma,   
  .id = "par"
...
knitr::kable(bind_rows(tabel_ranefi, tabel_fixefi),
             digits = 3,
             row.names = FALSE)
...
### Vergelijken beide methodes

De vergelijking in figuur @fig-compare-estimates is een beetje jonagolds en pink lady appels vergelijken. Het simpele model is geschat over alle cases heen, zonder rekening te houden met of de antwoorden van dezelfde respondent komen. Het individuele model daarentegen gebruikt enkel de cases waarvoor individuele schattingen aanwezig zijn, dus alles uit De Wijers en Gelinden verdwijnen - de helft van de casses.

Ondanks de verschillende modellen en gebruikte datasets liggen de geschatte beoordelingen van de ecosysteemdiensten heel dicht bij elkaar op enkele uitzonderingen na, namelijk `plant gathering`, `hunting`, `water quality` en `quality of living`. De ecosysteemdienst `bio materials` komt niet voor in de cases met individuele respondentinformatie.

De betrouwbaarheidsintervallen zijn een stuk breder voor de individuele data, maar de hoofdreden hiervoor is gewoon dat er heel wat minder data gebruikt is voor de schattingen. Sigma verschilt sterk tussen de methoden, maar dat is omdat in het individuele model sigma opgesplitst is in sigma en sd_nested.

```{r fig-compare-estimates}
#|fig-cap: "Comparison between the estimates of the simple model containing all cases, and the model zith only the 7 cases for which individual respondents could be identified (and using this for estimation)"
data_comp <- 
  bind_rows(
    bind_rows(tabel_ranef, tabel_fixef) |> mutate(model = "simple"),
    bind_rows(tabel_ranefi, tabel_fixefi) |> mutate(model = "individual")) |> 
  mutate(par = factor(par, levels = c("sigma", "sd_nested", "sd_intercept", levels(tabel_fixef$par))))
