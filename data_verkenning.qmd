...
<!--
vragen aan Francis:
* Hoe food&feed beter categoriseren
...
to do:
* vragen zijn nu puur op data beantwoord
...
data_orig <- data_raw |>
  rename_with(tolower) |> 
  select(case, respondent, ess_unique = esd_uniek, 
         ess_common = esd_common, ess_cluster = cluster, 
         score_original = score_uniek, score) |>
  mutate(f_score = factor(round(score)),
         ess_cluster = ifelse(ess_cluster == "Recreation & tourism",
                              "Recreation & Tourism", 
                              ess_cluster)) |>
  inner_join(ess_interpretation |>
...
tb_cluster <- tb_common |> 
  ...
  summarise(max_n_resp = max(n_resp),
            ...
            n_esd_common = n(), 
            n_esd_unique = sum(n_esd_unique),
            .groups = "drop") |> 
  mutate(total_unique = sum(n_esd_unique),
         total_common = sum(n_esd_common),
         fr_unique = n_esd_unique / total_unique,
         fr_common = n_esd_common / total_common,
         .by = c(case)) 
