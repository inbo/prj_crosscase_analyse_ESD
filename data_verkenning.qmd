...
Er zijn `r length(unique(data_ana$case_short))` gebieden waarin er aan verschillende personen gevraagd werd om verschillende ecosysteemdiensten te scoren van ongewenst tot zeer gewenst. Deze 14 gebieden worden in de analyse gerefereerd als `cases (case_short)`. De personen worden gerefereerd als `respondenten`.

### Terminologie

In ieder gebied werden verschillende ecosysteemdiensten bevraagd, sommige overlappen, andere hebben een heel andere naam voor hetzelfde en nog andere zijn opgesplitst in veel kleinere ecosysteemdiensten. De `r length(unique(data_ana$ess_unique)) ` bevraagde ecosysteemdiensten worden `unieke ecosysteemdiensten (ess_unique)` genoemd.

Deze worden geaggregeerd tot `r length(unique(data_ana$ess_short))` gemeenschappelijke ecosysteemdiensten `(ess_common, ess_short)`, die in de tekst ook gewoon als `ecosysteemdiensten` benoemd worden.

Deze ecosysteemdiensten worden dan nog eens verder samengevoegd in `r length(unique(data_ana$cluster_short))` ecosysteemdiensten clusters (cluster_short)` of kortbenoemd `clusters`.
...
In de volgende hoofdstukken wordt met een gewogen gemiddelde en een cumulatieve verdeling gewerkt om te modelleren.

Ik gebruik in de tekst ook consensus. De consensus wordt berekend als
...
Er wordt ook een kolom f_score gemaakt die een ordinale factor is van de score, met niveaus van 1 tot 5 (dus score + 2) zodat deze gebruikt kan worden in een ordinale analyse zoals een cumulatief logistisch model.

### Beperkingen in de dataset

In de data voor de cases "Gelinden" en "De Wijer" - deze laatste omvat 6 cases - is er geen link tussen respondent en antwoord, dus het is niet mogelijk een respondenteffect in rekening te brengen voor deze cases. Deze link zal dan ook niet gebruikt worden bij de analyse over de cases heen, al zal wel eens een analyse gebeuren voor deze cases apart.
...
```{r tbl-defcases}
#| tbl-cap: "Overzicht van de verschillende cases met hun aantal respondenten en beantwoorde ecosysteemdiensten"
...
if (knitr::is_html_output()) {
  ...
  # --- This is the fallback for PDF ---
  knitr::kable(tb_cases_tab,
...
```{r tbl-defess}
#| tbl-cap: "Definitie van de verschillende ecosysteemdiensten en hoe die geaggregeerd worden tot gemeenschappelijke ecosysteemdiensten en ecosysteemdienstclusters."
tb_ess <- data_ana |> 
...
my_colnames = c("ESS cluster", "ESS common",
                           "ESS short",
                           "Nr of ESS combined", 
                           "Nr of cases",
                           "Cases")
...
if (knitr::is_html_output()) {
  ...
  DT::datatable(tb_ess,
                ...
                # Note: 'options =' is the correct syntax
...
case_scores |>
  knitr::kable(
    caption = "Average desire of ecosystem services per case",
    col.names = c("case",
                  "different ESS",
                  "mean score",
                  "average consensus"))
...
ess_scores |>
  knitr::kable(caption = "Average desire of ecosystem services",
               col.names = c("ESS",
                             "different cases",
                             "mean score",
                             "average consensus"))
